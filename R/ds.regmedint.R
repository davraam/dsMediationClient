...
#' @export
#'
ds.regmedint <- function (data = NULL, yvar = NULL, avar = NULL, mvar = NULL, 
                          cvar = NULL, eventvar = NULL, a0 = NULL, a1 = 1, m_cde = 1, 
                          c_cond = NULL, mreg = NULL, yreg = NULL, interaction = TRUE, 
                          casecontrol = FALSE, na_omit = FALSE, exponentiate = FALSE, datasources = NULL) 
{
  ...
  if (!(is.list(datasources) && all(unlist(lapply(datasources, 
    ...
    stop("The 'datasources' were expected to be a list of DSConnection-class objects", 
         call. = FALSE)
  }
  if (is.null(yvar)) {
    stop(" Please provide the name of the yvar variable!", 
         call. = FALSE)
  }
  if (is.null(avar)) {
    stop(" Please provide the name of the avar of interest!", 
         call. = FALSE)
  }
  if (is.null(mvar)) {
    stop(" Please provide the name of the mvar variable!", 
         call. = FALSE)
  }
  if (is.null(data)) {
  ...
  if (is.null(mreg)) {
    stop(" Please provide the mediator regression type!", 
         call. = FALSE)
  }
  if (is.null(yreg)) {
    stop(" Please provide the outcome regression type!", 
         call. = FALSE)
  }
  defined.yvar <- dsBaseClient:::isDefined(datasources, paste0(data, 
                                                               "$", yvar))
  defined.avar <- dsBaseClient:::isDefined(datasources, paste0(data, 
                                                               "$", avar))
  defined.mvar <- dsBaseClient:::isDefined(datasources, paste0(data, 
                                                               "$", mvar))
  if (!is.null(cvar)) {
    lapply(cvar, function(x) {
      dsBaseClient:::isDefined(datasources, paste0(data, 
                                                   "$", cvar))
    })
  }
  if (!is.null(eventvar)) {
    defined.eventvar <- dsBaseClient:::isDefined(datasources, 
                                                 paste0(data, "$", eventvar))
  }
  yvar.name <- yvar
  avar.name <- avar
  mvar.name <- mvar
  ...
  if (!is.null(c_cond)) {
    c_cond.name <- paste0(as.character(c_cond), collapse = ",")
  }else{
    c_cond.name <- c_cond
  }
